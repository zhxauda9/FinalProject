...
router.get('/weather', async (req, res) => {
    ...
    try {
        const [weatherResponse, newsResponse, currencyResponse] = await Promise.all([
            ...
            axios.get(`https://newsapi.org/v2/everything?q=${city}&apiKey=${NEWS_API_KEY}`),
            axios.get(`https://v6.exchangerate-api.com/v6/${CURRENCY_API_KEY}/latest/USD`)
        ]);
        ...
        const airQuality = airData.stations?.[0]?.AQI || 'No data';
        ...
        res.json({
            ...
            currency: { USD_to_KZT: currencyData.KZT },
        });
    } catch (error) {
